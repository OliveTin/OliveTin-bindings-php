#!/usr/bin/env bash
# Run OliveTin PHP bindings release-line tests against supported PHP versions.
...
Options:
  --line <name>   Only run a release line (e.g. 1.x)
...
# Snapshot the current working tree (including uncommitted changes) without vendor/,
# so Docker can composer-install for the target PHP without touching the host tree.
prepare_working_copy() {
	...
	rsync -a \
		--exclude '.git/' \
		--exclude 'vendor/' \
		--exclude '.compat-worktrees/' \
		--exclude 'api-docs/' \
		--exclude '.phpunit.result.cache' \
		--exclude 'compat/.phpunit.result.cache' \
		"${ROOT}/" "${dir}/"
...
run_with_docker() {
	...
	# HEAD means the current working tree (including uncommitted changes).
	# Named refs/tags use an isolated git worktree of that revision.
	if [[ "$ref" == "HEAD" ]]; then
		tree="$(prepare_working_copy "${line//./-}-${php_version}-workdir")"
	else
		tree="$(prepare_worktree "$ref" "${line//./-}-${php_version}")"
